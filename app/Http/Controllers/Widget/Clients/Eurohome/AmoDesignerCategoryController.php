...
use App\Services\Amo\Analytics\Clients\Eurohome\AmoDesignerCategoryService;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use Illuminate\Support\Carbon;

class AmoDesignerCategoryController extends Controller
{
    public function data(Request $request, string $publicKey, AmoDesignerCategoryService $service): JsonResponse
    {
    ...
    public function leads(Request $request, string $publicKey, AmoDesignerCategoryService $service): JsonResponse
    {
        $installation = $this->installation($publicKey);
        $tz = $installation->account->timezone();
        [$from, $to] = $this->period($request, $tz);
    ...
    private function installation(string $publicKey): AmoAccountDashboardWidget
    {
        return AmoAccountDashboardWidget::query()
            ...
            ->whereHas('widget', fn ($q) => $q
                ->where('code', 'designer_category_dashboard')
                ->where('is_enabled', true))
            ->firstOrFail();
    }
